...
// ── BACKGROUND AUTO-ENRICHMENT WORKER ─────────────────────────────────────────
// Automatically resolves contacts for every unenriched artist, lowest listeners
// first. Runs one artist at a time to avoid IP bans. No clicks needed.

let _enrichBusy   = false
let _enrichPaused = false
let _enrichStats  = { processed: 0, found: 0, skipped: 0, current: null, remaining: 0 }

// IDs queued explicitly by "Run Enrichment" on selected artists — processed first
...
async function resetStuckArtists () {
  const { error } = await supabase
    .from('artists')
    .update({ contact_quality: 'none' })
    .eq('contact_quality', 'searching')
  if (!error) console.log('[AutoEnrich] Reset any stuck artists from previous run')
...
async function autoFlushJunk () {
  ...
  // 3. Recheck artists that have an IG handle but no follower count yet.
  //    Re-runs resolve.py on them (same pipeline) to get ig_followers and detect 100K+.
  //    Does ONE artist per flush cycle so it doesn't block anything.
  try {
    const { data, error } = await supabase
      .from('artists')
      .select('id, name, instagram, platform')
      .eq('status', 'new')
    ...
    if (!error && data?.length) {
      const artist = data[0]
      console.log(`[AutoFlush] IG recheck for ${artist.name} (@${artist.instagram})`)
      await _recheckViaResolve(artist)
    }
  } catch (err) {
    console.error('[AutoFlush] IG recheck error:', err.message)
  }
}

// Re-run resolve.py for a single artist to get their IG follower count.
// If 100K+ detected, deletes them. Otherwise saves the follower count.
async function _recheckViaResolve (artist) {
  return new Promise((done) => {
    const resolvePath = path.join(__dirname, '../discovery/resolve.py')
    const args = [resolvePath, '--json', '--name', artist.name, '--platform', artist.platform || '', '--ig', artist.instagram]
    if (artist.profile_url) args.push('--profile-url', artist.profile_url)
    const py = spawn(PYTHON_CMD, args)
    let stdout = ''
    const killTimer = setTimeout(() => { py.kill(); done() }, 90000)
    ...
    py.on('close', async () => {
      ...
      if (result.skip || (result.ig_followers && result.ig_followers > 100000)) {
        ...
        await supabase.from('artists').update({ contact_quality: 'skip', updated_at: new Date().toISOString() }).eq('id', artist.id)
        console.log(`[AutoFlush] Flag ${artist.name}: ${reason}`)
      } else if (result.ig_followers) {
        await supabase.from('artists').update({ ig_followers: result.ig_followers }).eq('id', artist.id)
        console.log(`[AutoFlush] Updated ${artist.name}: ${result.ig_followers.toLocaleString()} IG followers`)
      }
      done()
    })
    py.on('error', () => { clearTimeout(killTimer); done() })
  })
}

async function pickNextUnenriched () {
  // Priority queue: drain artists explicitly selected by the user first
  while (_priorityQueue.length > 0) {
    const id = _priorityQueue.shift()
    const { data } = await supabase.from('artists').select('*').eq('id', id).single()
    // Skip if already enriched or deleted since being queued
    if (data && (data.contact_quality === 'none' || data.contact_quality == null)) {
      return data
    }
  }

  // Spotify first → Deezer second → Last.fm third
  // Within each platform: newest session first, then lowest followers
  for (const platform of ['spotify', 'deezer', 'lastfm']) {
    const { data, error } = await supabase
      .from('artists')
      .select('*')
      ...
      .or('ig_followers.is.null,ig_followers.lte.100000')
      .order('discovered_at', { ascending: false, nullsFirst: false })
      .order('followers',     { ascending: true,  nullsFirst: false })
      .limit(1)
    if (!error && data?.[0]) return data[0]
  }

  // Final fallback — any platform (handles contact_quality column missing too)
  const { data, error } = await supabase
    .from('artists')
    .select('*')
    .or('contact_quality.is.null,contact_quality.eq.none')
  ...
  // Fallback when contact_quality column is missing (schema not yet migrated)
  console.warn('[AutoEnrich] contact_quality column missing — using fallback query. Run SQL migration in Supabase.')
...
async function getRemainingCount () {
  const { count, error } = await supabase
    ...
    .or('contact_quality.is.null,contact_quality.eq.none')
  if (error) {
    // Fallback count when column missing
...
async function autoEnrichWorker () {
  ...
  if (_enrichPaused) { _enrichBusy = false; return }
  _enrichBusy = true

  try {
    const artist = await pickNextUnenriched()

    if (!artist) {
      _enrichStats.current   = null
      _enrichStats.remaining = 0
      _enrichBusy = false
      // All done — check again in 10 minutes in case new leads were added
      setTimeout(autoEnrichWorker, 10 * 60 * 1000)
      return
    }

    _enrichStats.current   = artist.name
    _enrichStats.remaining = await getRemainingCount()
    console.log(`[AutoEnrich] → ${artist.name} (${(artist.followers || 0).toLocaleString()} followers) — ${_enrichStats.remaining} remaining`)

    // Mark as in-progress (best-effort — ok to fail if column missing)
    try { await supabase.from('artists').update({ contact_quality: 'searching' }).eq('id', artist.id) } catch {}
    ...
    await new Promise((done) => {
      ...
      py.on('close', async () => {
        ...
        if (!result) {
          console.error(`[AutoEnrich] Parse failed for ${artist.name}:`, stderr.slice(0, 300))
          try { await supabase.from('artists').update({ contact_quality: 'contactless' }).eq('id', artist.id) } catch {}
          done(); return
        }

        if (result.skip || result.ig_followers > 100000) {
          const reason = result.skip_reason || `${(result.ig_followers || 0).toLocaleString()} IG followers`
          console.log(`[AutoFlush] Flag ${artist.name}: ${reason}`)
          await supabase.from('artists').update({ contact_quality: 'skip', updated_at: new Date().toISOString() }).eq('id', artist.id)
          _enrichStats.skipped++
          _enrichStats.current = null
        } else {
          const updates = {
          ...
          // Try full save first; if columns are missing strip them and retry with core fields only
          let { error: workerSaveErr } = await supabase.from('artists').update(updates).eq('id', artist.id)
          if (workerSaveErr) {
            console.warn(`[AutoEnrich] Save error for ${artist.name}: ${workerSaveErr.message}`)
            const coreUpdates = {
              instagram:  updates.instagram,
              email:      updates.email,
              updated_at: updates.updated_at,
            }
            const { error: retryErr } = await supabase.from('artists').update(coreUpdates).eq('id', artist.id)
            if (retryErr) console.error(`[AutoEnrich] Core save also failed for ${artist.name}: ${retryErr.message}`)
          }
          const nFound = [updates.instagram, updates.email, updates.facebook, updates.phone].filter(Boolean).length
          if (nFound) _enrichStats.found++
          console.log(`[AutoEnrich] ✓ ${artist.name}: ${updates.contact_quality}${updates.instagram ? ` @${updates.instagram}` : ''}${updates.email ? ` ${updates.email}` : ''}`)
        }
      ...
      py.on('error', async (err) => {
        clearTimeout(killTimer)
        console.error(`[AutoEnrich] Python error for ${artist.name}:`, err.message)
        await supabase.from('artists').update({ contact_quality: 'contactless' }).eq('id', artist.id)
        done()
      })
    })

  } catch (err) {
    console.error('[AutoEnrich] Worker error:', err.message)
  }

  _enrichBusy = false
  if (!_enrichPaused) setTimeout(autoEnrichWorker, 2000)
}

// GET /api/enrich/status — live progress for the dashboard
app.get('/api/enrich/status', (req, res) => {
  res.json({ ..._enrichStats, busy: _enrichBusy, paused: _enrichPaused })
})

// POST /api/enrich/pause — pause/resume the background enrich worker
app.post('/api/enrich/pause', (req, res) => {
  _enrichPaused = true
  console.log('[AutoEnrich] Paused by user')
  res.json({ success: true, paused: true })
})

app.post('/api/enrich/resume', (req, res) => {
  _enrichPaused = false
  console.log('[AutoEnrich] Resumed by user')
  if (!_enrichBusy) setTimeout(autoEnrichWorker, 100)
...
app.listen(PORT, async () => {
  ...
  await autoFlushJunk()
  // Re-run auto-flush every 3 minutes to quickly catch 100K+ IG and blocked-region artists
  setInterval(autoFlushJunk, 3 * 60 * 1000)
  setTimeout(autoEnrichWorker, 5000)
})
